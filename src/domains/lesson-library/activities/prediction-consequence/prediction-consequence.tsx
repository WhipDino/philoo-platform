...
import styles from "./prediction-consequence.module.css";
...
export function PredictionConsequence<TChoice extends string>({
  ...
  return (
    <section className={styles.root} data-prediction-consequence>
      <fieldset disabled={disabled || isPending || choicesLocked}>
        <legend className={styles.prompt}>{prompt}</legend>
        <div className={styles.choices} data-prediction-choices>
          {choices.map((choice) => (
      ...
      {isLocked ? (
        ...
        >
          {matched && matchedStatus ? (
            <p className={styles.status} data-prediction-status>
              {matchedStatus}
            </p>
          ) : null}
          ...
          {consequence ? (
            <p className={styles.consequence} data-consequence>
              {consequence}
            </p>
          ) : null}
          <p className={styles.feedback} data-feedback>
            {matched ? matchedFeedback : unmatchedFeedback}
          </p>
          {showRetry ? (
            <button
