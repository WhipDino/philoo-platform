...
import { HERACLITUS_ASSETS } from "./heraclitus-assets";
import { HERACLITUS_CLASSIFICATION_CONFIG } from "./heraclitus-classification-config";
import {
  HERACLITUS_FOLIO_BEATS,
  HERACLITUS_SCENE_META,
  type HeraclitusFolioBeat,
} from "./heraclitus-content";
import {
  HERACLITUS_JOURNEY_STAGES,
  type HeraclitusSceneId,
} from "./heraclitus-journey";
import styles from "./heraclitus-scene.module.css";

type HeraclitusSceneProps = {
  sceneId: HeraclitusSceneId;
};

export function HeraclitusScene({ sceneId }: HeraclitusSceneProps) {
  return (
    <FolioBeatsLesson
      sceneId={sceneId}
      beats={HERACLITUS_FOLIO_BEATS[sceneId]}
    />
  );
}

function isExerciseBeat(beat: HeraclitusFolioBeat) {
  return beat.kind === "prediction" || beat.kind === "classification";
}
...
function FolioBeatsLesson({
  ...
  sceneId: HeraclitusSceneId;
  beats: readonly HeraclitusFolioBeat[];
}) {
  const meta = HERACLITUS_SCENE_META[sceneId];
  ...
  const panelAsset =
    beat?.kind === "story-panel" ? HERACLITUS_ASSETS[beat.imageKey] : null;
  const currentBeat =
    HERACLITUS_JOURNEY_STAGES.findIndex((stage) =>
  ...
  return (
    <>
      <PhilooStoryShell
        ...
        totalBeats={HERACLITUS_JOURNEY_STAGES.length}
        labelledBy="heraclitus-title"
        phase="idle"
        className={
        ...
        showSoftFrame={false}
        journey={{
          lessonTitle: "Heráclito de Éfeso",
          currentSceneId: sceneId,
          stages: HERACLITUS_JOURNEY_STAGES,
          storageKey: "philoo:journey:heraclitus",
        }}
      >
        <PhilooFolioStage
          eyebrow={meta.eyebrow}
          title={meta.title}
          titleId="heraclitus-title"
        ...
        >
          {beat.kind === "classification" ? (
            ...
            >
              <GuidedClassificationExercise
                config={HERACLITUS_CLASSIFICATION_CONFIG}
            ...
            >
              {beat.imageKey ? (
                <figure className={styles.predictionVisual}>
                  <Image
                    src={HERACLITUS_ASSETS[beat.imageKey].src}
                    alt={HERACLITUS_ASSETS[beat.imageKey].alt}
                    width={HERACLITUS_ASSETS[beat.imageKey].width}
                    height={HERACLITUS_ASSETS[beat.imageKey].height}
            ...
            <PhilooNarrativeComposition
              className={
                beat.kind === "story-panel"
                  ? `${storyLayout.storyStage} ${styles.heraclitusGuideComposition}`
                  : `${storyLayout.narrativeComposition} ${styles.heraclitusGuideComposition}`
              }
      ...
      {canRenderBriefing && isExerciseBeat(beat) && "briefing" in beat ? (
        <PhilooActivityBriefing
          ...
          guideCharacterId="heraclitus"
          guidePoseId={beat.briefing.guidePoseId}
          guideLabel="Heráclito te acompanha"
          demonstration={
            beat.kind === "prediction" ? (
              <PhilooChoiceDemonstration />
